...
Configuration (environment, set per attachment by the kernel):
    ...
    MCPHUB_SSO_TOKEN            the acting user's SSO access token (required —
                                forwarded as X-MCPHUB-SSO-TOKEN; the hub
                                resolves identity and permissions from it)
...
class HubProxy:
    def __init__(self, url: str, access_key: str, secret_key: str, sso_token: str) -> None:
        self.url = url
...
def main() -> int:
    ...
    HubProxy(url, access_key, secret_key, sso_token).run()
    return 0
