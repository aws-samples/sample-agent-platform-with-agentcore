...
- **Bedrock 直连**：以容器的 IAM 角色调用 `bedrock:InvokeModel*`，无长期密钥，
  模型 ID 用 `global.` 前缀的跨区推理配置。
- **LLM 网关（如 LiteLLM）**：内核从 Secrets Manager 读 API key（密钥从不进
  镜像、不进注册表、不进 DynamoDB；控制面里只存密钥的*名字*），流量从固定
  NAT EIP 出去，网关侧做源 IP 白名单；模型白名单、预算、按团队成本归属都在
  网关层执行。
...
作为示例项目，这套平台在几处为演示便利做了取舍，安全团队评估生产化时应当知道：

1. **Bedrock 直连模式下 `bedrock:InvokeModel*` 资源是 `*`**：跨区推理配置
   横跨多个区域，无法收紧到单区；可用 SCP/权限边界按推理配置 ARN 约束，或把模型管控
   完全放在 LLM 网关层。
2. **加密密钥默认为 AWS 托管**：合规要求 CMK 时，S3/DynamoDB/AgentCore 资源
...
3. **不用的能力可以整体移除**：内置工具（Code Interpreter/Browser）、Bedrock
   ...
   删除即生效。
4. **所有平台角色都可加权限边界**，保证未来任何代码改动都不能越过边界扩权。

给安全团队一个可核对的交付物：`cdk synth --all` 生成的 CloudFormation 模板里
包含本文和 permissions.md 描述的每一条 IAM 语句，可以逐条比对，无需授予任何
推测性权限。
